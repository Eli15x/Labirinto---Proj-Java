...
//adicionarPath(linha,coluna); //os valores passado como parametro que é a posicao inicial...

System.out.println("Linha Path :"+this.pathLinha);
System.out.println("Coluna Path :"+this.pathColuna);
if(inicio()){
   System.out.println("INICIOOOO ");
	//COMO É A PRIMEIRA VEZ NAO É NESCESSARIO CONFERIR O PATH pois ele ainda nao existe.
...
if(coluna < this.TotalColuna-1)
	...
	if(conferirEValidarPosicaoABaixo(linha,coluna)){  //como linha é 0 em qualquer valor para coluna será possivel verificar a posicao abaixo
    	if(!(colocouPasso)){
        	try{
				colocarPasso();
				//adicionarPath(linha,coluna);
		...
		}else  //se o passo foi colocado e é valido
		{
...
//&&(coluna == this.TotalColuna-1)){
  ...
  if(coluna < this.TotalColuna-1){
	...
	} //FALTANDO O NORMAL CASO ESTEJA ENTRE ... QUE SERÁ < QUE COLUNA-1
...
public boolean inicio(){
	...
	if((this.posicaoColunaInicial == this.coluna)&&(this.posicaoLinhaInicial == this.linha))
		return true;
...
/*public void posicoes(int aux)
{
 ...
 /* public int getLinhaValida1()
  ...
  public boolean posicaoValida() throws Exception
  {
	  ...
	  if(labirinto[this.linha][this.coluna] == '#')
	  {
	    Evalida = false;
      }
	  else
